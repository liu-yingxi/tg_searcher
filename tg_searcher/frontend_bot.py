...
# Imports from project structure (with fallbacks)
try:
...
# --- BotFrontendConfig and FakeRedis classes remain unchanged ---
...
"""
    ...
    # --- start, _track_user_activity, _callback_handler, _normal_msg_handler, _chat_ids_from_args, _admin_msg_handler, _search, _download_history methods remain largely unchanged, except for Redis error handling improvements already made ---
    # (Skipping unchanged methods for brevity)
    async def start(self):
        ...
        try:
            ...
            if self._admin_id:
                 try:
                     status_msg = await self.backend.get_index_status(4000 - 100)
    ...
    async def _callback_handler(self, event: events.CallbackQuery.Event):
        ...
        try:
            ...
            page_key = f'{redis_prefix}query_page:{bot_chat_id}:{result_msg_id}' # Page key

            # --- 处理翻页和筛选 ---
            if action == 'search_page' or action == 'search_filter':
                 # Get current context, handle expiration
                 ...
                 current_page_num = int(self._redis.get(page_key) or 1)

                 if current_query is None:
                     try: await event.edit("搜索信息已过期，请重新搜索。")
                     except Exception: pass
                     self._redis.delete(query_key, chats_key, filter_key, page_key); await event.answer("搜索已过期。"); return

                 # Determine new page and filter
                 new_page_num, new_filter = current_page_num, current_filter
                 if action == 'search_page':
                      ...
                      if temp_filter != current_filter: # Filter changed
                           new_filter = temp_filter
                           new_page_num = 1 # Reset to page 1
                      # else filter is same, new_page_num remains current_page_num

                 # Update Redis context if needed
                 if not self._cfg.no_redis:
                     try:
                         pipe = self._redis.pipeline()
                         pipe.set(page_key, new_page_num, ex=3600) # Update page number
                         if new_filter != current_filter: pipe.set(filter_key, new_filter, ex=3600) # Update filter if changed
                         pipe.execute()
                         ...
                         if "MISCONF" in str(e) and not self._cfg.no_redis: # Check if not already fallen back
                            self._redis = FakeRedis(); self._cfg.no_redis = True
                            logger.error("Falling back to FakeRedis due to MISCONF error during callback context update.")
                 ...
                 except rpcerrorlist.MessageIdInvalidError: await event.answer("消息已被删除。")
            ...
            # --- 处理选择聊天 ---
                 ...
                 try:
                      ...
                      await event.answer("缓存服务暂时遇到问题，请稍后重试。", alert=True)
                 except Exception as e: self._logger.error(f"Error in select_chat callback: {e}", exc_info=True); await event.answer("选择对话时出错。", alert=True)
    ...
    async def _normal_msg_handler(self, event: events.NewMessage.Event):
        ...
        if not text or text.startswith('/start'):
            ...
            else: await event.reply('没有正在监控的对话。请先使用管理员命令添加。')
        ...
        if text.startswith('/help'): await event.reply(self.HELP_TEXT_ADMIN, parse_mode='markdown'); return
            ...
            try:
                ...
                if msg:
                    ...
                    except Exception: await event.reply(error_text)
                else: await event.reply(error_text)
             ...
             try:
                 ...
                 if isinstance(self._redis, FakeRedis):
                     ...
                     try:
                         ...
                         await event.reply(f"获取使用统计时出错: Redis 服务器配置错误 (MISCONF)。"); return
                     except RedisConnectionError as e:
                         logger.error(f"Redis connection error during usage check: {e}")
                         await event.reply(f"获取使用统计时出错: 无法连接到 Redis。"); return
             ...
             await self._normal_msg_handler(event) # Fallback to normal handler
        ...
        try:
            ...
            if msg and not self._cfg.no_redis:
                try:
                    ...
                    else: pipe.delete(f'{prefix}query_chats:{bcid}:{mid}')
                    pipe.execute()
                except RedisResponseError as e:
    ...
    async def _download_history(self, event: events.NewMessage.Event, chat_id: int, min_id: int, max_id: int):
        ...
        try:
            ...
            try:
                ...
                prog_msg = None
            except Exception: await self.bot.send_message(event.chat_id, msg, parse_mode='html')
    ...
    async def _get_selected_chat_from_reply(self, event: events.NewMessage.Event) -> Optional[Tuple[int, str]]:
        ...
        if self._cfg.no_redis: return None
        ...
        if res:
            try:
                 ...
                 name = await self.backend.translate_chat_id(cid)
                 return cid, name
            except ValueError: self._redis.delete(key); return None
            except EntityNotFoundError: return int(res), f"未知对话 ({res})"
            except (RedisResponseError, RedisConnectionError) as e:
    ...
    async def _register_commands(self):
        ...
        if self._admin_id:
            try: admin_peer = await self.bot.get_input_entity(self._admin_id)
                 ...
                 self._logger.warning(f"Could not get input entity for admin ID {self._admin_id} directly. Trying get_entity.")
    ...
    async def _render_response_text(self, result: SearchResult, used_time: float) -> str:
        ...
        for i, hit in enumerate(result.hits, 1):
            try:
                ...
                # --- 构建消息头 (修正) ---
                # 确保只添加必要的元素：序号、粗体标题、时间戳
                hdr_parts = [f"<b>{i}. {html.escape(title)}</b>"]
                if isinstance(msg.post_time, datetime):
                    hdr_parts.append(f'<code>[{msg.post_time.strftime("%y-%m-%d %H:%M")}]</code>') # 用 code 包裹时间
                else:
                ...
                if hit.highlighted:
                    ...
                    if len(plain_highlighted) <= self.MAX_TEXT_DISPLAY_LENGTH:
                        ...
                        display_text = html.escape(brief_content(plain_highlighted, self.MAX_TEXT_DISPLAY_LENGTH)) # 超长，去高亮截断
                elif msg.content: # 没有高亮，但有原文
                    display_text = html.escape(brief_content(msg.content, self.MAX_TEXT_DISPLAY_LENGTH))
                # 注意：此处不再有 else 分支添加 "(空消息)"

                # 3. 如果前面没加文件链接，且 URL 存在，则添加通用跳转链接
                if not link_added and msg.url:
                    sb.append(f'<a href="{html.escape(msg.url)}">跳转到消息</a>\n')

                # 4. 添加处理后的文本 (当且仅当 display_text 非空时)
                if display_text:
